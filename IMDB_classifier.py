...
model.add(layers.Dense(16, activation='relu', input_shape=(10000, ))) # 第一层，16个神经元，ReLU激活函数
model.add(layers.Dense(16, activation='relu')) # 第二层，16个神经元，ReLU激活函数
model.add(layers.Dense(1, activation='sigmoid')) # 输出层，1个神经元，Sigmoid激活函数
...
model.compile(
    optimizer=optimizers.RMSprop(learning_rate=0.001),  # 优化器，使用RMSprop，学习率为0.001
    loss=losses.binary_crossentropy,  # 损失函数，使用二元交叉熵
    metrics=[metrics.binary_accuracy]  # 评估指标，使用二元准确率
)
