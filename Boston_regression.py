...
for i in range(k):
    ...
    model.fit(partical_train_data, partical_train_targets, epochs=num_epochs, 
        batch_size=1, verbose=0)  # 训练模型
    # partical_train_data: 训练数据
    # partical_train_targets: 训练目标
    # epochs: 训练轮次
    # batch_size: 每批次训练样本数
    # verbose: 日志显示模式，0为静默模式
    # 在验证数据上评估模型
    val_mse, val_mae = model.evaluate(val_data, val_targets, verbose=0)  # 在验证数据上评估模型，返回均方误差和平均绝对误差
    all_scores.append(val_mae)  # 将当前折的平均绝对误差添加到所有分数列表中
